...
def get_auth_diagnostics() -> dict:
    ...
    return {
        ...
        "auth_mode": "User authorization / Statement API" if user_token else "App service principal / Statement API",
...
def _warehouse_id() -> str:
    ...
    Databricks Apps resources normally expose DATABRICKS_WAREHOUSE_ID.  For
    compatibility with older deployments, also derive the ID from an HTTP path.
    """
    warehouse_id = (
        os.getenv("DATABRICKS_WAREHOUSE_ID")
        or os.getenv("CFO_WAREHOUSE_ID")
    ...
    if WAREHOUSE_HTTP_PATH:
        ...
        if marker in WAREHOUSE_HTTP_PATH:
            return WAREHOUSE_HTTP_PATH.split(marker, 1)[1].strip("/")

    raise RuntimeError(
        "No SQL warehouse configured. Add the SQL Warehouse as a Databricks "
        "App resource and expose DATABRICKS_WAREHOUSE_ID."
    )
...
def _workspace_base_url() -> str:
    ...
    if not host:
        raise RuntimeError("Databricks workspace host is unavailable in the App runtime.")
    if not host.startswith(("http://", "https://")):
        host = f"https://{host}"
    return host


def _statement_api_headers() -> dict:
    """Build authorization headers for Statement Execution.

    Prefer Databricks Apps user authorization. This means Unity Catalog and
    SQL Warehouse permissions are evaluated as the signed-in user.  If no
    forwarded user token exists, fall back to the App service principal so the
    source remains portable to environments where that principal is authorized.
    """
    user_token = _request_header("x-forwarded-access-token")
    if user_token:
        return {
    ...
    try:
        sdk_headers = cfg.authenticate()
    except Exception as exc:
        raise RuntimeError(
            "No forwarded user token was received and App service-principal "
            f"authentication could not be initialized: {exc}"
        ) from exc

    headers = dict(sdk_headers or {})
    headers["Content-Type"] = "application/json"
    if "Authorization" not in headers:
        raise RuntimeError("Databricks authentication did not provide an Authorization header.")
    return headers


def _api_json(method: str, path: str, payload: dict | None = None, timeout: int = 65) -> dict:
    """Call one Databricks REST endpoint and return decoded JSON."""
    url = f"{_workspace_base_url()}{path}"
    body = None if payload is None else json.dumps(payload).encode("utf-8")
    req = urllib_request.Request(
        url=url,
        data=body,
        headers=_statement_api_headers(),
        method=method,
    )
    ...
    try:
        ...
        try:
            ...
            detail = str(exc)
        raise RuntimeError(
            f"Databricks Statement API HTTP {exc.code}: {detail}"
        ) from exc
    except urllib_error.URLError as exc:
        raise RuntimeError(
            f"Databricks Statement API network error: {exc.reason}"
        ) from exc


def _statement_error(payload: dict) -> str:
    status = payload.get("status") or {}
    err = status.get("error") or {}
    code = err.get("error_code") or err.get("sql_state") or "UNKNOWN"
    message = err.get("message") or "Statement execution failed."
    return f"{code}: {message}"


def _wait_for_statement(statement_id: str, initial_payload: dict) -> dict:
    ...
    while True:
        ...
        if state in {"FAILED", "CANCELED", "CLOSED"}:
            raise RuntimeError(f"SQL statement {state}: {_statement_error(payload)}")
        if time.time() >= deadline:
            raise RuntimeError(
                f"Timed out waiting for SQL statement {statement_id}; last state={state or 'UNKNOWN'}."
            )
...
def _coerce_statement_dataframe(df: pd.DataFrame, columns_meta: list[dict]) -> pd.DataFrame:
    """Restore useful pandas types from Statement Execution JSON strings."""
    if df.empty:
        return df
    ...
    for meta in columns_meta:
        ...
        type_name = str(meta.get("type_name") or "").upper()
        ...
        if type_name in {
            ...
            df[name] = df[name].map(
                lambda value: None if value is None else str(value).lower() == "true"
            )
...
def run_query(query: str) -> pd.DataFrame:
    """Execute governed SQL through Databricks Statement Execution REST API.

    This intentionally bypasses the SQL connector's Thrift OpenSession path,
    which can be blocked in governed Databricks Apps environments even when
    user authorization and SQL Warehouse permissions are valid.
    """
    ...
    if not statement_id:
        raise RuntimeError(f"Statement API returned no statement_id: {response}")

    response = _wait_for_statement(statement_id, response)
    ...
    # INLINE result sets can contain multiple chunks. Fetch every remaining
    # chunk so callers receive the same logical result they got from the SQL
    # connector previously.
    while next_chunk_index is not None:
        chunk = _api_json(
            "GET",
            f"/api/2.0/sql/statements/{statement_id}/result/chunks/{next_chunk_index}",
            timeout=30,
        )
    ...
    return _coerce_statement_dataframe(df, columns_meta)
